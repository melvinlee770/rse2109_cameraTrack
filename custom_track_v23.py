"""
Staging Zone Tracker v22 - Perspective-Corrected Dimensions + Dashboard + Overlap Region
=========================================================================================
...
Added: Overlap region support — pallets in the overlap zone between
       two cameras use a strict 50% overlap threshold (skip point checks)
...
Usage:
    ...
    python custom_track_v22.py --model best.pt --overlap-file overlap.json
"""
...
OVERLAP_THRESH=0.5  # 50% threshold for pallets in the overlap region
...
YOLO_CLASS_TO_PALLET={"uk_pallet_empty":"UK", "uk_pallet_loaded":"UK", "cage":"UK", "us_pallet_empty":"US", "us_pallet_loaded":"US",
                      "eu_pallet_empty":"UK", "eu_pallet_loaded":"UK"}  # display name aliases

# Rename wrong annotation names for display without retraining
...
def load_overlap_region(fp):
    """
    Load the overlap region polygon from a JSON file.
    The file should contain a list of [x, y] points defining the overlap boundary.
    Example overlap.json:
    {
        "overlap_pts": [[400, 100], [500, 100], [500, 600], [400, 600]],
        "note": "Strip between Camera A and Camera B zones"
    }
    ...
    try:
        with open(fp,'r') as f: d=json.load(f)
        pts=np.array(d["overlap_pts"],dtype=np.float32)
        print(f"[INFO] Overlap region loaded from: {fp} ({len(pts)} points)")
        return pts
    except Exception as e: print(f"[WARN] Load overlap region failed: {e}"); return None

def point_in_polygon(px, py, polygon):
    """Quick check if a point is inside a polygon using cv2.pointPolygonTest."""
...
def compute_real_dims_perspective(zone_pts, aruco_corners, tag_cm):
    ...
    tag_m = tag_cm / 100.0
    ppm_x = np.mean(edges_h) / tag_m
    ppm_y = np.mean(edges_v) / tag_m

    if ppm_x <= 0 or ppm_y <= 0:
        return None, None, None

    real_w = TEMP_SZ / ppm_x
    real_h = TEMP_SZ / ppm_y
...
def _compute_overlap_ratio(box, zp):
    """
    Compute the pixel overlap ratio between a bounding box and a zone polygon.
    Returns overlap_pixels / bounding_box_pixels.
    """
    x1,y1,x2,y2=box
    box_pts=np.array([[x1,y1],[x2,y1],[x2,y2],[x1,y2]],dtype=np.float32)
    box_area=(x2-x1)*(y2-y1)
    if box_area<=0: return 0.0
    zx_min=int(max(0,min(zp[:,0].min(),x1)));zy_min=int(max(0,min(zp[:,1].min(),y1)))
    zx_max=int(max(zp[:,0].max(),x2)+1);zy_max=int(max(zp[:,1].max(),y2)+1)
    ...
    zp_s=(zp-[zx_min,zy_min]).astype(np.int32).reshape((-1,1,2))
...
def inzone(box, zp, overlap_thresh=0.3, overlap_region=None, overlap_region_thresh=0.5):
    """
    Check if a bounding box is inside the zone.

    Two modes of operation:
    1. NORMAL (pallet is NOT in the overlap region):
       Uses the fast 3-stage check: center point → bottom-center → 30% overlap.
       Same as original logic.

    2. OVERLAP REGION (pallet center IS inside the overlap region):
       Skips the quick point checks entirely.
       Goes straight to the mask-based overlap calculation.
       Uses a stricter threshold (default 50%) so that a pallet straddling
       the boundary between two zones can only belong to one zone — whichever
       side has the majority (>50%) of its area.

    Args:
        box:                    (x1,y1,x2,y2) bounding box
        zp:                     zone polygon (4 corner points)
        overlap_thresh:         threshold for normal mode (default 0.3 = 30%)
        overlap_region:         polygon defining the overlap region (or None)
        overlap_region_thresh:  threshold for overlap mode (default 0.5 = 50%)
    """
    ...
    # Check if pallet center is in the overlap region
    if overlap_region is not None and point_in_polygon(cx, cy, overlap_region):
        # OVERLAP MODE: skip point checks, use strict 50% overlap only
        return _compute_overlap_ratio(box, zp) >= overlap_region_thresh

    # NORMAL MODE: fast 3-stage check (unchanged from original)
    ...
    # Stage 2: bottom-center point
    bcx=cx;bcy=float(y2)
    if cv2.pointPolygonTest(zp2,(float(bcx),bcy),False)>=0: return True

    # Stage 3: overlap ratio (30%)
    return _compute_overlap_ratio(box, zp) >= overlap_thresh
...
def calc_util_fixed(besz,dst,dets,zcam,M,ppx,ppy,overlap_region=None):
    ...
    for det in dets:
        ...
        if not inzone(box,zcam,overlap_region=overlap_region): continue
...
def ddets(f,dets,zp,overlap_region=None):
    iz=oz=0
    for det in dets:
        box=det["box"];cf=det["conf"];cn=det["cls_name"];pt=det["pallet_type"]
        ins=inzone(box,zp,overlap_region=overlap_region);x1,y1,x2,y2=box
...
def main():
    pa=argparse.ArgumentParser(description="Staging Zone v22 - Perspective Corrected + Dashboard + Overlap Region")
    ...
    pa.add_argument("--overlap-file", type=str, default=None,
                    help="JSON file defining the overlap region polygon (default: None = no overlap region)")
    pa.add_argument("--overlap-thresh", type=float, default=OVERLAP_THRESH,
                    help="Overlap threshold for pallets in the overlap region (default: 0.5 = 50%%)")
    args=pa.parse_args()
    ...
    # --- Load overlap region ---
    overlap_region = load_overlap_region(args.overlap_file)
    if overlap_region is not None:
        print(f"[INFO] Overlap region active: {len(overlap_region)} points, threshold: {args.overlap_thresh*100:.0f}%")
    else:
        print(f"[INFO] No overlap region defined (single-camera mode)")
    ...
    while True:
        ...
        if lvp is not None and lM is not None and ldst is not None:
            up,zpx,opx,counts,cls_counts=calc_util_fixed((lbw,lbh),ldst,fd,lvp,lM,lppx,lppy,overlap_region=overlap_region)
            us_val=UTIL_A*up+(1-UTIL_A)*us_val
            if lppm and lppm>0 and zpx>0: oam=lra*(opx/zpx) if lra else None
        elif lvp is not None:
            for det in fd:
                if inzone(det["box"],lvp,overlap_region=overlap_region):
        ...
        # === DRAW: Overlap region (if defined) ===
        if overlap_region is not None:
            orpd=overlap_region.astype(int).reshape((-1,1,2))
            ov_or=frame.copy();cv2.fillPoly(ov_or,[orpd],(0,200,200));cv2.addWeighted(ov_or,0.1,frame,0.9,0,frame)
            cv2.polylines(frame,[orpd],True,(0,200,200),1,cv2.LINE_AA)

        iz,oz=ddets(frame,fd,lvp,overlap_region=overlap_region)
        ...
        if show_ui:
            ...
            ol_st="ACTIVE" if overlap_region is not None else "NONE"
            cv2.putText(frame,f"Overlap:{ol_st}({args.overlap_thresh*100:.0f}%)",(px+10,y),cv2.FONT_HERSHEY_SIMPLEX,0.38,(0,200,200),1);y+=lh
        ...
        if show_be and lM is not None and lvp is not None:
            ...
            for det in fd:
                box=det["box"];pt=det["pallet_type"];cn=det["cls_name"]
                if not inzone(box,lvp,overlap_region=overlap_region): continue

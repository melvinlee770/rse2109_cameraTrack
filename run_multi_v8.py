...
PALLET_TYPES={
    "US":{"width_m":0.075,"depth_m":0.065,"grid_w":15,"grid_h":13,"label":"US(15x13)","color":(0,165,255)},
    "EU":{"width_m":0.085,"depth_m":0.055,"grid_w":17,"grid_h":11,"label":"EU(17x11)","color":(255,165,0)}}
# 1 grid cell = 0.5cm = 0.005m
# US pallet = 15 x 13 cells = 7.5cm x 6.5cm
# EU pallet = 17 x 11 cells = 8.5cm x 5.5cm
YOLO_CLASS_TO_PALLET={"uk_pallet_empty":"EU", "uk_pallet_loaded":"EU", "cage":"EU",
                      "us_pallet_empty":"US", "us_pallet_loaded":"US",
                      "eu_pallet_empty":"EU", "eu_pallet_loaded":"EU"}
...
def save_zone(fp,pts,ppm,area,pa,rw=None,rh=None):
    d={"zone_pts":pts.tolist(),"ppm":float(ppm) if ppm else None,"real_area_m2":float(area) if area else None,
       ...
       "real_width_m":float(rw) if rw else None,"real_height_m":float(rh) if rh else None}
    with open(fp,'w') as f: json.dump(d,f,indent=2)

def load_zone(fp):
    if not os.path.exists(fp): return None,None,None,None,None,None
    try:
        with open(fp,'r') as f: d=json.load(f)
        return (np.array(d["zone_pts"],dtype=np.float32), d.get("ppm"), d.get("real_area_m2"),
                d.get("pixel_area"), d.get("real_width_m"), d.get("real_height_m"))
...
def estimate_orientation_be(box, M):
    """
    Determine whether a pallet is horizontal or vertical in bird's-eye space
    by warping all 4 YOLO bbox corners and comparing the resulting width vs
    height.  Returns (be_width, be_height, swapped).

    YOLO bounding boxes are ALWAYS axis-aligned in camera space — they never
    rotate with the object.  Therefore the old approach (measuring the angle
    of the top edge after warp) always returned ≈ 0° and never triggered a
    swap.

    The correct signal is the **aspect ratio of the warped bbox**.  When the
    physical pallet is rotated past 45° the YOLO box becomes taller than
    wide (because the box stretches to enclose the rotated object).  After
    perspective-correcting into bird's-eye space this tall-vs-wide property
    is preserved:

        warped width > warped height  →  pallet is horizontal  →  no swap
        warped height >= warped width  →  pallet is vertical    →  swap W↔H

    The 45° decision point maps exactly to the aspect-ratio crossover at 1:1.
    """
    x1, y1, x2, y2 = box
    tl = warp_pt((x1, y1), M)
    tr = warp_pt((x2, y1), M)
    ...
    # Measure the warped width (avg of top and bottom edges)
    w_top = np.hypot(tr[0] - tl[0], tr[1] - tl[1])
    w_bot = np.hypot(br[0] - bl[0], br[1] - bl[1])
    be_w = (w_top + w_bot) / 2.0

    # Measure the warped height (avg of left and right edges)
    h_left  = np.hypot(bl[0] - tl[0], bl[1] - tl[1])
    h_right = np.hypot(br[0] - tr[0], br[1] - tr[1])
    be_h = (h_left + h_right) / 2.0

    swapped = be_h > be_w   # taller than wide → vertical → swap
    return be_w, be_h, swapped

def stamp_fixed_footprint_px(mask, center_be, box, M, ppx, ppy, pt):
    """
    Stamp a fixed-size axis-aligned rectangle on a pixel mask.
    Uses bird's-eye aspect ratio to decide orientation:
    if warped bbox is taller than wide → swap width/height (90° snap).
    Used by calc_util_fixed for utilization calculation.
    """
    _, _, swapped = estimate_orientation_be(box, M)
    if pt and pt in PALLET_TYPES:
        pw_m = PALLET_TYPES[pt]["width_m"]
        ph_m = PALLET_TYPES[pt]["depth_m"]
    else:
        pw_m = 0.080; ph_m = 0.060
    if swapped:
        pw_m, ph_m = ph_m, pw_m
    # Convert meters to bird's-eye pixels
    pw_px = int(pw_m * ppx)
    ph_px = int(ph_m * ppy)
    cx, cy = int(center_be[0]), int(center_be[1])
    x1 = max(0, cx - pw_px // 2); y1 = max(0, cy - ph_px // 2)
    x2 = min(mask.shape[1], cx + pw_px // 2); y2 = min(mask.shape[0], cy + ph_px // 2)
    if x2 > x1 and y2 > y1:
        cv2.rectangle(mask, (x1, y1), (x2, y2), 255, -1)

def get_pallet_center_be(box, M):
    """Warp the center of a bounding box to bird's-eye space."""
    x1, y1, x2, y2 = box
    cx = (x1 + x2) / 2
    cy = (y1 + y2) / 2
    return warp_pt((cx, cy), M)

def calc_util_fixed(besz, dst, dets, zcam, M, ppx, ppy, boundary_strip=None, boundary_thresh=0.5):
    """
    Hybrid utilization calculation:
      - US/EU pallets: use FIXED dimensions (US=7.5x6.5cm, EU=8.5x5.5cm)
        with 45° orientation snapping
      - Unknown objects: use WARPED YOLO BOUNDING BOX (actual detected footprint)
    """
    bw, bh = besz
    zm = np.zeros((bh, bw), dtype=np.uint8)
    cv2.fillPoly(zm, [dst.astype(np.int32).reshape((-1, 1, 2))], 255)
    za = cv2.countNonZero(zm)
    if za == 0: return 0.0, 0, 0, {"US": 0, "EU": 0, "unknown": 0}, {}
    pm = np.zeros((bh, bw), dtype=np.uint8)
    counts = {"US": 0, "EU": 0, "unknown": 0}; cls_counts = {}
    for det in dets:
        ...
        if pt and pt in PALLET_TYPES:
            counts[pt] += 1; cls_counts[cn] = cls_counts.get(cn, 0) + 1
            # Known pallet → stamp fixed dimensions
            center_be = get_pallet_center_be(box, M)
            stamp_fixed_footprint_px(pm, center_be, box, M, ppx, ppy, pt)
        else:
            counts["unknown"] += 1
            # Unknown object → warp YOLO bounding box
            x1, y1, x2, y2 = box
    ...
    return (oa / za) * 100.0, za, oa, counts, cls_counts

def compute_occupancy_grid(besz, dst, dets, zcam, M, ppx, ppy, real_w, real_h, cell_size=0.005,
                           boundary_strip=None, boundary_thresh=0.5):
    """
    Abstract top-down grid — pallet positions without perspective distortion.

    Key improvements over earlier versions:
    • Uses round() instead of int() to avoid systematic truncation drift.
    • Maps centre → grid via (grid_cols), not (grid_cols-1).
    • **Neighbour snapping**: after computing the ideal grid position, checks
      every already-placed pallet.  If the gap between them is small (≤ half
      a pallet width) the new block is snapped to be exactly adjacent — zero
      gap.  This handles the sub-cell rounding errors that come from the
      bird's-eye perspective warp on a ~20° camera.
    • Spiral collision resolution only fires *after* snapping, so overlapping
      YOLO boxes push blocks to the nearest free spot instead of creating
      artificial gaps.
    """
    bw, bh = besz

    grid_cols = max(1, int(round(real_w / cell_size)))
    grid_rows = max(1, int(round(real_h / cell_size)))
    grid_cols = min(grid_cols, 500)
    grid_rows = min(grid_rows, 500)

    # ── Extract the actual zone rectangle from dst ──
    dst_x_min = float(dst[:, 0].min())
    dst_y_min = float(dst[:, 1].min())
    dst_x_max = float(dst[:, 0].max())
    dst_y_max = float(dst[:, 1].max())
    dst_w = max(dst_x_max - dst_x_min, 1.0)
    dst_h = max(dst_y_max - dst_y_min, 1.0)

    grid = [[0] * grid_cols for _ in range(grid_rows)]
    det_labels = []
    pallet_info = {}

    # ── Collect in-zone pallets ──
    pallets = []
    for det in dets:
        box = det["box"]; pt = det["pallet_type"]; cn = det["cls_name"]
        if not inzone(box, zcam, boundary_strip=boundary_strip,
                      boundary_thresh=boundary_thresh):
            continue

        center_be = get_pallet_center_be(box, M)

        # Normalise relative to the zone rectangle (not the canvas)
        nx = (center_be[0] - dst_x_min) / dst_w
        ny = (center_be[1] - dst_y_min) / dst_h
        nx = max(0.0, min(1.0, nx))
        ny = max(0.0, min(1.0, ny))

        _, _, swapped = estimate_orientation_be(box, M)
        if pt and pt in PALLET_TYPES:
            bw_ = PALLET_TYPES[pt]["grid_w"]
            bh_ = PALLET_TYPES[pt]["grid_h"]
        else:
            bw_ = 16; bh_ = 12  # average of US(15x13) and EU(17x11)
        if swapped:
            bw_, bh_ = bh_, bw_

        pallets.append({"nx": nx, "ny": ny, "bw": bw_, "bh": bh_,
                        "pt": pt, "cn": cn})

    pallets.sort(key=lambda p: (p["ny"], p["nx"]))

    # ── Helpers ──
    def _collides(cs, rs, pw, ph):
        for rr in range(rs, min(grid_rows, rs + ph)):
            for cc in range(cs, min(grid_cols, cs + pw)):
                if grid[rr][cc] > 0:
                    return True
        return False

    # Track placed block rectangles for neighbour snapping:
    #   [(c_start, r_start, pw, ph), ...]
    placed_rects = []
    ...
    for p in pallets:
        pallet_id += 1
        pw = p["bw"]; ph = p["bh"]
        if pw > grid_cols or ph > grid_rows:
            continue

        # ── Step 1: compute ideal centre cell (round, not truncate) ──
        target_c = int(round(p["nx"] * grid_cols))
        target_r = int(round(p["ny"] * grid_rows))

        # Top-left corner from centre, clamped inside grid
        c_start = max(0, min(grid_cols - pw, target_c - pw // 2))
        r_start = max(0, min(grid_rows - ph, target_r - ph // 2))

        # ── Step 2: neighbour snapping ──
        # Close small gaps caused by perspective-warp rounding errors.
        # SNAP_GAP  = max gap on the snap axis   (4 cells = 2.0cm)
        # SNAP_CROSS = max misalignment on cross axis (3 cells = 1.5cm)
        # Previous value of 8 was too aggressive (pulled distant pallets).
        # Previous value of 2/1 was too tight (missed slightly-offset pairs).
        SNAP_GAP   = 4   # 2.0cm — covers warp rounding + slight physical gap
        SNAP_CROSS = 3   # 1.5cm — allows for real-world horizontal misalignment
        best_snap = None
        best_snap_dist = float('inf')

        for (pc, pr, ppw, pph) in placed_rects:
            # --- Try snapping to the RIGHT of the existing block ---
            snap_c = pc + ppw
            gap_c = abs(c_start - snap_c)
            gap_r = abs(r_start - pr)
            if gap_c <= SNAP_GAP and gap_r <= SNAP_CROSS:
                sc = max(0, min(grid_cols - pw, snap_c))
                sr = max(0, min(grid_rows - ph, pr))
                d = abs(c_start - sc) + abs(r_start - sr)
                if d < best_snap_dist and not _collides(sc, sr, pw, ph):
                    best_snap = (sc, sr); best_snap_dist = d

            # --- Try snapping to the LEFT of the existing block ---
            snap_c = pc - pw
            if snap_c >= 0:
                gap_c = abs(c_start - snap_c)
                if gap_c <= SNAP_GAP and gap_r <= SNAP_CROSS:
            ...
            # --- Try snapping BELOW the existing block ---
            snap_r = pr + pph
            gap_r2 = abs(r_start - snap_r)
            gap_c2 = abs(c_start - pc)
            if gap_r2 <= SNAP_GAP and gap_c2 <= SNAP_CROSS:
                sc = max(0, min(grid_cols - pw, pc))  # align lefts
                sr = max(0, min(grid_rows - ph, snap_r))
                d = abs(c_start - sc) + abs(r_start - sr)
                if d < best_snap_dist and not _collides(sc, sr, pw, ph):
                    best_snap = (sc, sr); best_snap_dist = d

            # --- Try snapping ABOVE the existing block ---
            snap_r = pr - ph
            if snap_r >= 0:
                gap_r2 = abs(r_start - snap_r)
                if gap_r2 <= SNAP_GAP and gap_c2 <= SNAP_CROSS:
                    sc = max(0, min(grid_cols - pw, pc))
                    sr = max(0, min(grid_rows - ph, snap_r))
                    d = abs(c_start - sc) + abs(r_start - sr)
                    if d < best_snap_dist and not _collides(sc, sr, pw, ph):
                        best_snap = (sc, sr); best_snap_dist = d
        ...
        if best_snap is not None:
            c_start, r_start = best_snap

        # ── Step 3: spiral collision resolution (fallback) ──
        if _collides(c_start, r_start, pw, ph):
            placed = False
            max_shift = max(grid_cols, grid_rows)
            for dist in range(1, max_shift):
                if placed:
                    break
                for dc in range(-dist, dist + 1):
                    for dr in range(-dist, dist + 1):
                        ...
                        if nc < 0 or nr < 0:
                            continue
                        if nc + pw > grid_cols or nr + ph > grid_rows:
                            continue
                        if not _collides(nc, nr, pw, ph):
                            c_start = nc; r_start = nr
                            placed = True; break
                    if placed:
                        break

        # ── Stamp the block ──
        r_end = min(grid_rows, r_start + ph)
        c_end = min(grid_cols, c_start + pw)
        for r in range(r_start, r_end):
            for c in range(c_start, c_end):
                if grid[r][c] == 0:
                    grid[r][c] = pallet_id

        placed_rects.append((c_start, r_start, pw, ph))
        pallet_info[pallet_id] = {"type": p["pt"] or "unknown", "label": p["cn"]}

        label_c = (c_start + c_end) // 2
        label_r = (r_start + r_end) // 2
        det_labels.append({"col": label_c, "row": label_r,
                           "label": p["cn"], "type": p["pt"] or "unknown"})

    return grid, det_labels, grid_cols, grid_rows, pallet_info
...
def camera_loop(zone_id, camera_idx, model_path, args, dashboard, stop_event):
    ...
    lrw=0;lrh=0  # real zone dimensions in meters
    bstrip=None
    ...
    saved_pts,saved_ppm,saved_area,saved_pa,saved_rw,saved_rh = load_zone(zone_file)
    if saved_pts is not None:
        ...
        # Restore real dimensions and recompute bird's-eye transform
        if saved_rw and saved_rh and saved_rw > 0 and saved_rh > 0:
            lrw=saved_rw; lrh=saved_rh
            lM,ldst,lbw,lbh,lppx,lppy=compute_be(lvp,lrw,lrh,BEW,BEH)
            print(f"[ZONE {zone_id}] Restored dimensions: {lrw:.4f}m x {lrh:.4f}m = {lra:.4f}m²")
    ...
    while not stop_event.is_set():
        ...
        counts={"US":0,"EU":0,"unknown":0};cls_counts={};oam=None;up=0.0;iz=oz=0
        ...
        grid_data = None; grid_labels = []; grid_cols = 0; grid_rows = 0; grid_pallet_info = {}
        if lvp is not None and lM is not None and ldst is not None and lrw > 0 and lrh > 0:
            grid_data, grid_labels, grid_cols, grid_rows, grid_pallet_info = compute_occupancy_grid(
                (lbw,lbh), ldst, fd, lvp, lM, lppx, lppy,
                real_w=lrw, real_h=lrh, cell_size=args.grid_cell_size,
                boundary_strip=bstrip, boundary_thresh=boundary_thresh)
        ...
        dashboard.feed(zone_id, {
            ...
            "grid_cols":        grid_cols,
            "grid_rows":        grid_rows,
            "grid_pallet_info": {str(k): v for k, v in grid_pallet_info.items()},
        })
        ...
        if lvp is not None:
            ...
            # --- Draw edge dimensions (perspective-corrected) ---
            edge_lengths_m = []
            if lM is not None and lppx and lppy:
                for i in range(4):
                    ...
                    edge_lengths_m.append(edge_m)
                    txt=f"{edge_m:.2f}m"
                    tsz,_=cv2.getTextSize(txt,cv2.FONT_HERSHEY_SIMPLEX,0.55,2)
                    tx=mid[0]-tsz[0]//2; ty=mid[1]-10
                    cv2.rectangle(frame,(tx-3,ty-tsz[1]-3),(tx+tsz[0]+3,ty+5),(0,0,0),-1)
                    cv2.putText(frame,txt,(tx,ty),cv2.FONT_HERSHEY_SIMPLEX,0.55,(255,255,255),2)
                # Recompute area from displayed edges — only when NOT locked
                # When locked, use the saved lra to keep it consistent across restarts
                el_r = [round(e, 2) for e in edge_lengths_m]
                avg_w = (el_r[0] + el_r[2]) / 2  # avg of top + bottom
                avg_h = (el_r[1] + el_r[3]) / 2  # avg of left + right
                if not zone_locked:
                    lra = round(avg_w * avg_h, 4)
            elif lppm and lppm>0:
                for i in range(4):
                    p1=lvp[i]; p2=lvp[(i+1)%4]
                    mid=((p1+p2)/2).astype(int)
                    edge_m=np.linalg.norm(p2-p1)/lppm
                    edge_lengths_m.append(edge_m)
                    txt=f"{edge_m:.2f}m"
                    tsz,_=cv2.getTextSize(txt,cv2.FONT_HERSHEY_SIMPLEX,0.55,2)
                    tx=mid[0]-tsz[0]//2; ty=mid[1]-10
                    ...
                    cv2.putText(frame,txt,(tx,ty),cv2.FONT_HERSHEY_SIMPLEX,0.55,(255,255,255),2)
                el_r = [round(e, 2) for e in edge_lengths_m]
                avg_w = (el_r[0] + el_r[2]) / 2
                avg_h = (el_r[1] + el_r[3]) / 2
                if not zone_locked:
                    lra = round(avg_w * avg_h, 4)
            ...
            if lra:
                ...
                area_txt=f"Area: {lra:.4f} m2"
                atsz,_=cv2.getTextSize(area_txt,cv2.FONT_HERSHEY_SIMPLEX,0.5,1)
                ay=zc[1]+25 if not zone_locked else zc[1]+35
                cv2.rectangle(frame,(zc[0]-atsz[0]//2-4,ay-atsz[1]-4),(zc[0]+atsz[0]//2+4,ay+4),(0,0,0),-1)
                cv2.putText(frame,area_txt,(zc[0]-atsz[0]//2,ay),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,255),1)

            # --- Draw corner labels ---
            for i,(pt_c,lb) in enumerate(zip(lvp,["TL","TR","BR","BL"])):
                x,y=int(pt_c[0]),int(pt_c[1])
                cv2.circle(frame,(x,y),5,(0,0,255),-1)
                cv2.putText(frame,lb,(x+8,y-8),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,255),2)
        ...
        # --- Store latest annotated frame as JPEG for dashboard snapshot download ---
        _, jpg_buf = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 85])
        dashboard.store_snapshot_image(zone_id, jpg_buf.tobytes())
        ...
        if key==ord('q'): stop_event.set(); break
            ...
            if not zone_locked and lvp is not None:
                zone_locked=True;save_zone(zone_file,lvp,lppm,lra,lpa,rw=lrw,rh=lrh)
                if boundary_edge!="none": bstrip=compute_boundary_strip(lvp,boundary_edge,boundary_width)
                print(f"[ZONE {zone_id}] LOCKED & SAVED")
            elif zone_locked: zone_locked=False;zs.reset();print(f"[ZONE {zone_id}] UNLOCKED")
        elif key==ord('r'):
            lvp=lppm=lra=lM=None;lpa=0;us_val=0;lrw=0;lrh=0;zs.reset();zone_locked=False;bstrip=None
            delete_zone_file(zone_file);print(f"[ZONE {zone_id}] RESET")
        elif key==ord('c'):
            rm={"CENTER":"INNER","INNER":"OUTER","OUTER":"CENTER"}[rm]
            lvp=lra=lppm=lM=None;lpa=0;us_val=0;lrw=0;lrh=0;zs.reset();zone_locked=False;bstrip=None
...
def main():
    ...
    pa.add_argument("--grid-cell-size", type=float, default=0.005,
                    help="Grid cell size in meters (default: 0.005 = 5mm x 5mm)")
    args = pa.parse_args()
    ...
    print(f"  Grid cell: {args.grid_cell_size*1000:.1f}mm x {args.grid_cell_size*1000:.1f}mm")

...
def inzone(box,zp,overlap_thresh=0.3):
    """
    Check if a bounding box is inside the zone using overlap ratio.
    Instead of only checking the center point (which fails with tilted cameras),
    this computes the overlap area between the bounding box and zone polygon.
    If overlap / bounding_box_area >= threshold, it's considered in-zone.
    Also returns True if bottom-center or center is inside the zone.
    """
    if zp is None: return False
    zp2=zp.astype(np.float32).reshape((-1,1,2))
    x1,y1,x2,y2=box
    # Quick check: center or bottom-center inside zone
    cx=(x1+x2)/2;cy=(y1+y2)/2
    if cv2.pointPolygonTest(zp2,(float(cx),float(cy)),False)>=0: return True
    bcx=cx;bcy=float(y2)  # bottom-center (where pallet sits on ground)
    if cv2.pointPolygonTest(zp2,(float(bcx),bcy),False)>=0: return True
    # Overlap check: compute intersection area between bbox and zone polygon
    box_pts=np.array([[x1,y1],[x2,y1],[x2,y2],[x1,y2]],dtype=np.float32)
    box_area=(x2-x1)*(y2-y1)
    if box_area<=0: return False
    # Use mask-based intersection
    # Get bounding rect of both shapes for a small ROI
    ...
    return (inter/box_area)>=overlap_thresh
